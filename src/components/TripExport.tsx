...
export function TripExport() {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      if (format === 'csv') {
        exportToCSV(trips as TripData[]);
      } else {
        exportToPDF(trips as TripData[]);
      }
  ...
  const exportToCSV = (trips: TripData[]) => {
    // CSV headers
    const headers = [
      'Trip ID',
      'Date',
    ...
    const rows = trips.map(trip => [
      ...
      trip.status || 'N/A',
      'Driver Name', // TODO: Join with profiles
  ...
  const exportToPDF = (trips: TripData[]) => {
      ...
      <CardContent className="space-y-4">
        <div className="grid md:grid-cols-2 gap-4">
          <div>
            ...
            <Select value={format} onValueChange={(v: 'csv' | 'pdf') => setFormat(v as typeof format)}>
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
          ...
          <div>
            ...
            <Select value={period} onValueChange={(v: 'month' | 'quarter' | 'year' | 'all') => setPeriod(v as typeof period)}>
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
